...
def consultar_demandas():
    ...
    query = '''
        SELECT d.id, u.nome as unidade_nome, u.id as unidade_id, ts.descricao as tipo_servico,
               d.descricao, s.descricao as status, d.data
        FROM demandas d
        ...
        if 'criar_ordem_servico' in request.form:
            ...
            if demanda_ids and unidade_id:
                return redirect(url_for('criar_ordem_servico',
                                        demanda_ids=','.join(demanda_ids),
                                        unidade_id=unidade_id))
            else:
                mensagem = "Nenhuma demanda selecionada"
                return render_template('consultar_demandas.html',
                                       unidades=unidades,
...
def criar_ordem_servico():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('''
            INSERT INTO ordem_servico (unidade_id, data_previsao, observacoes)
            VALUES (%s, %s, %s)
        ...
        for demanda_id in demanda_ids:
            cursor.execute('''
                INSERT INTO ordem_servico_demandas (ordem_servico_id, demanda_id)
                VALUES (%s, %s)
            ''', (ordem_servico_id, demanda_id))

        for tecnico_id in tecnicos_ids:
            cursor.execute('''
                INSERT INTO ordem_servico_tecnicos (ordem_servico_id, tecnico_id)
                VALUES (%s, %s)
    ...
    return render_template('criar_ordem_servico.html',
                           unidade=unidade,
                           demandas=demandas,
                           tecnicos=tecnicos)
...
def ver_ordem_servico(ordem_servico_id):
    ...
    return render_template('ver_ordem_servico.html',
...
def gerar_pdf(ordem_servico_id):
    try:
        ...
        cursor.execute('''
            SELECT os.id as ordem_servico_id, u.nome as unidade_nome,
                   os.data_criacao, os.data_previsao, os.observacoes
        ...
        rendered = render_template('ordem_servico_pdf.html',
                                   ordem_servico=ordem_servico,
                                   demandas=demandas,
                                   tecnicos=tecnicos)
...
def consultar_ordens_servico():
    ...
    return render_template('consultar_ordens_servico.html',
